        ...
        while True:
            try:
                ...
                if rdt_recv.seq == expected_seq:
                    ...
                    # If expected segment, then save in file
                    fd.write(rdt_recv.data)
                    ack_segm = UDPSeg(seq=0, ack=expected_seq, flag=ACK, data=b'')
                    sock.sendto(ack_segm.encapsulation(), addr)
                    expected_seq += 1

                # Else send double ACK for last acknowledged segment
                else:
                    ack_segm = UDPSeg(seq=0, ack=expected_seq - 1, flag=ACK, data=b'')
                    sock.sendto(ack_segm.encapsulation(), addr)
...
def server_handshake(server_isn, sock, addr_ref, max_attempts=10):
    ...
    segment = UDPSeg(seq=server_isn, ack=client_isn + 1, flag=SYN | ACK, data=b'')
    bytes_to_send = segment.encapsulation()
